...
# looks for the best ngram table. If typed ngram is not found there then 
# its first token is deleted and process proceed with the next down table.
# Do the same in case there are not three words in only one table.
...
bigramNextWord <- as.data.table(readRDS("data/nextWords_2_sbkf.rds"))
trigramNextWord <- as.data.table(readRDS("data/nextWords_3_sbkf.rds"))
fourgramNextWord <- as.data.table(readRDS("data/nextWords_4_sbkf.rds"))
