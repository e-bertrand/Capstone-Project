############################################################################
# Creating the table with the most probable continuation after two words
# from the trigram model. If no trigram for that sequence exists, stupid backoff
...
# As distance for basic model is 2, lamba is raised to the power of 2
freqWords <- ngram_1[order(freq, decreasing = TRUE)][ngram != "<s/>"][1:freqLimit]
residualWords <- freqWords$ngram
residualScores <- freqWords$prob * (0.4 ^ 2)
...
for (i in 1:numBases){
  ...
  # For each base we get the last word
  base2 <- baseList[i]
  tokens <- unlist(strsplit(base2, "\\s+"))
  base1 <- tokens[2]
...
next_words_3 <- data.table(ngram = baseList,
                           w_1 = w_1,
                           w_2 = w_2,
                           w_3 = w_3)
...
saveRDS(next_words_3,  paste0("nextWords_3_sbkf.rds"))
